...
function GridSquareLR() {
  ...
  const handleChange = async (event: SelectChangeEvent) => {
    ...
    const squareIdParam = squareId ?? ''
    const preds = await getPredictions(event.target.value, squareIdParam)
    setPredictions(preds)
    if (event.target.value === 'overall') {
      const preds = await getOverallPredictions(squareIdParam)
      setPredictions(preds)
    } else {
      const preds = await getPredictions(event.target.value, squareIdParam)
      setPredictions(preds)
      ...
      <Stack direction="row" spacing={2}>
        <Container maxWidth="lg" content="center" style={{ width: '100%', paddingTop: '50px' }}>
          <Card variant="outlined">
            ...
            >
              <img src={`${url}/gridsquares/${squareId}/gridsquare_image`} alt="Grid square" />
            ...
            <CardActions>
              ...
              <IconButton
                ...
                onClick={() =>
                  navigate({
                    to: '/acquisitions/$acqId/grids/$gridId/square/$squareId/predictions',
                    params: { acqId, gridId, squareId },
                  })
                }
              >
                <InsightsIcon />
...
export const Route = createFileRoute('/acquisitions/$acqId/grids/$gridId/squares/$squareId')({
  component: GridSquareLR,
})
