...
const getPredictions = async (modelName: string, gridSquareId: string) => {
        ...
        const holePredictionsMap = new Map<string, number>(holePredictions.map((elem: Prediction) => [elem.foilhole_uuid, elem.value]));
        return holePredictionsMap;
    }

const getOverallPredictions = async (gridSquareId: string) => {
  const holePredictionsResponse = await fetch(`${apiUrl()}/gridsquare/${gridSquareId}/overall_prediction`);
  const holePredictions = await holePredictionsResponse.json();
  const holePredictionsMap = new Map<string, number>(holePredictions.map((elem: Prediction) => [elem.foilhole_uuid, elem.value]));
  return holePredictionsMap;
}
                ...
                <Card variant="outlined">
                    ...
                    <svg viewBox='0 0 2880 2046' style={{ "position": "absolute", "top": 0, "left": 0 }}>
                        ...
                        <circle
                            ...
                            r={predictions?.get(foilHole.uuid) ? (1.5*foilHole.diameter*(predictions.get(foilHole.uuid)-predictionMin)/(predictionMax-predictionMin))*maxWidth / 2: foilHole.diameter / 2}
                            fillOpacity={0.5}
                            fill={latentRep ?
                            ...
                            strokeOpacity={1}
                            stroke={(foilHole.uuid === selectedHole) ? "orange": showPredictions ? predictions?.get(foilHole.uuid) ? predictions.get(foilHole.uuid) >= 0.5 ? "green": "red": "gray": "gray"}
                    ...
                    <CardActions>
                        ...
                        <IconButton aria-label="add-label" style={{"display": "flex"}} onClick={() => navigate(`../../square/${params.squareId}/predictions`, { relative: "path"})}>
                          <InsightsIcon />
                        </IconButton>
