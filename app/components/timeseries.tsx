import { Avatar, Box, Grid, Slider, Switch, List, ListItem, ListItemText, ListItemAvatar } from "@mui/material";
...
export const TimeSeriesChart = ({ xData, yData }: { xData: number[] | Date[], yData: number[] }) => {
    ...
    const average = yData.reduce((a: number, b: number) => a + b, 0) / yData.length;
    const averageLastTenPercent = yData.slice(-(yData.length / 10)).reduce((a: number, b: number) => a + b, 0) / (yData.length / 10);
    const sdev = Math.sqrt(yData.map((x: number) => Math.pow(x - average, 2)).reduce((a: number, b: number) => a + b) / yData.length);
    const current = yData.slice(-1)[0];
    ...
    return <Box sx={{ borderColor: "#b927d9", borderWidth: 5, borderRadius: "10px", padding: "20px" }}>
